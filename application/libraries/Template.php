...
class CI_Template {
   ...
   function initialize($props)
   {
      ...
      if (isset($props['template']) 
         && (file_exists(APPPATH .'views/'. $props['template']) or file_exists(APPPATH .'views/'. $props['template'] . EXT) or $props['parse_template']))
      {
   ...
   /**
    * Dynamically include META tags in the template. Use the "_metas" variable name in templates!
    *
    * @param string $type Can be 'http-equiv' (Provides an HTTP header for the information in the content attribute) or 'name' (Provides a name for the information in the content attribute)
    * @param string $value Can be one from 'content-type', 'content-style-type', 'expires', 'refresh', 'set-cookie', if type is http-equiv, otherwise one from 'author', 'description', 'keywords', 'generator', 'revised'
    * @param string $content Specifies the content of the meta information
    * @param boolean $is_xhtml Specifies if the output should be in XHTML or HTML format
    * @return TRUE on success, FALSE otherwise
    */
    function add_meta($type, $value, $content, $is_xhtml=TRUE) {
      $success = TRUE;
      $meta = NULL;
      $http_equiv_values = array('content-type', 'content-style-type', 'expires', 'refresh', 'set-cookie');
      $name_values = array('author', 'description', 'keywords', 'generator', 'revised');

      switch ($type) {
        case 'http-equiv':
          if (in_array($value, $http_equiv_values)) {
            $meta = '<meta http-equiv="'.$value.'" content="'.$content.'"'.($is_xhtml ? ' /': '').'>';
          }
          else {
            $success = FALSE;
          }
        ...
        case 'name':
          if (in_array($value, $name_values)) {
            $meta = '<meta name="'.$value.'" content="'.$content.'"'.($is_xhtml ? ' /': '').'>';
          }
          else {
            $success = FALSE;
          }
